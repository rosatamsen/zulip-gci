...
There are three types of tasks in this category, each corresponding to one of
the Task Types below.

### Task Type A: Create user avatars
...
* Create a directory `user-avatars/<username>`, where `<username>` is
...
* Submit a pull request to the `zulip/zulip-gci-submissions` repository, with
  title "Visual Art: Create a/an <theme> avatar set." Include a link to the
...
### Task Type B: Create a custom animated reaction
...
### Task Type C: Create a standalone animation
